...
inline std::wstring TEXT(const char* src)
{
...
std::wstring MediaInfo(const boost::filesystem::path& path, const std::shared_ptr<core::media_info_repository>& media_info_repo)
{
	if(boost::filesystem::is_regular_file(path))
	{
		...
		{		
			...
			auto str = relativePath.replace_extension("").native();	// on Linux it returns std::string

/*			if(str[0] == L'\\' || str[0] == '/') {
				//memcpy(dstr, str.begin()+1, str.size()-1);
				str = std::string(str.begin() + 1, str.end());
			}
*/
			auto media_info = media_info_repo->get(path.wstring());
			std::wstring quote = L"\"", space = L" ";
			...
			return std::wstring() 
					...
					+ space + sizeStr
					+ space	+ writeTimeWStr
					+ space	+ boost::lexical_cast<std::wstring>(media_info.duration)
					+ space	+ boost::lexical_cast<std::wstring>(media_info.time_base.numerator()) + L"/" + boost::lexical_cast<std::wstring>(media_info.time_base.denominator())
					+ L"\r\n";
		}
	}
	return L"";
...
//			auto dir = relativePath.parent_path().native();	changed to following
			auto dir = relativePath.parent_path().wstring();
			auto file = boost::to_upper_copy(relativePath.filename().wstring());
//			relativePath = boost::filesystem::path(dir + boost::lexical_cast<std::basic_string<char>>(L"/") 
//					+ boost::lexical_cast<std::basic_string<char>>(file));
			relativePath = boost::filesystem::path(dir + L"/" + file);
	
			auto str = relativePath.replace_extension(L"").native();
			boost::trim_if(str, boost::is_any_of("\\/"));
			std::wstring prepstring = L"\"" + widen(str) + L"\" " + sizeWStr + L" " + writeTimeWStr + L"\r\n";

			replyString << prepstring;
		}
	}
...
{
	if(!pClientInfo_) {
		CASPAR_LOG(info) << L" !pClientInfo_";
		return;
	}
...
bool LoadbgCommand::DoExecute()
{
	...
	try
	{
		...
		if (uri_tokens[0] == L"route")
		{
			CASPAR_LOG(info) << L"route ==>> ";
			pFP = RouteCommand::TryCreateProducer(*this, _parameters.at_original(0));
		}

		if (pFP == frame_producer::empty())
		{
			CASPAR_LOG(info) << L"create_producer... " << _parameters.get_original_string();
			pFP = create_producer(GetChannel()->mixer()->get_frame_factory(GetLayerIndex()), _parameters);
		}

		CASPAR_LOG(info) << L"create_transition_producer =>> " << _parameters.begin()[0] << _parameters.end()[0];
...
bool PlayCommand::DoExecute()
{
	try
	{
		if(!_parameters.empty())
		{
			...
			//Comment out the original function, it has no real impact
			caspar::IO::ClientInfoPtr cptr = GetClientInfo();
			lbg.SetClientInfo(cptr);
...
/*
	Comment out CGCommand handling for now since we're not be dealing with flash...
...
//	std::wstring data;			//_parameters[4] alt. _parameters[5]
	...
	// layer = _ttoi(_parameters[1].c_str()); use atoi() api
	layer = atoi((const char *)_parameters[1].c_str());
	...
	{	//read label
		...
		if(_parameters.size() > 4 && _parameters[4].length() > 0)	//read play-on-load-flag
			bDoStart = (_parameters[4][0]==L'1') ? true : false;
		else 
		{
			SetReplyString(TEXT("402 CG ERROR\r\n"));
			return false;
		...
		bDoStart = (_parameters[3][0]==L'1') ? true : false;
	}
	else 
	{
		SetReplyString(TEXT("403 CG ERROR\r\n"));
...
bool DataCommand::DoExecuteStore() 
{
	...
	std::strcpy (cstr, (const char*)filename.c_str());
//	std::wofstream datafile(boost::lexical_cast<std::basic_string<char>>(filename.c_str()));
	std::wofstream datafile(cstr);
...
//			std::wstring prepreply(boost::lexical_cast<std::basic_string<wchar_t>>(str + "\r\n"));
			std::wstring prepreply(boost::lexical_cast<std::basic_string<wchar_t>>(str.c_str()));
			prepreply.append(L"\r\n");

			replyString << prepreply;
		}
	}
...
bool ThumbnailCommand::DoExecuteList()
{
	...
	{			
		if(boost::filesystem::is_regular_file(itr->path()))
		{
			...
			auto str = relativePath.replace_extension(L"").native();
			std::wstring newstr(boost::lexical_cast<std::wstring>(str.c_str()));
			if(str[0] == '\\' || str[0] == '/')
				newstr = std::wstring(str.begin() + 1, str.end());
			...
			replyString << L"\"" << newstr << L"\" " << widen(mtime_readable) << L" " << file_size << L"\r\n";
		}
	}
...
bool VersionCommand::DoExecute()
{
	...
	if(_parameters.size() > 0)
	{
		...
		//else 
		//if(_parameters[0] == L"TEMPLATEHOST")
		//	replyString = TEXT("201 VERSION OK\r\n") + flash::get_cg_version() + TEXT("\r\n");
		//else 
		if(_parameters[0] != L"SERVER")
