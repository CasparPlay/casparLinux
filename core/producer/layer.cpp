...
public:
	...
	{		
		//background_ = producer; //kill error message
	...
	{			
		if(background_ != frame_producer::empty())
		{
			...
			//background_ 		= make_safe<frame_producer>(frame_producer::empty());
			frame_number_		= 0;
			auto_play_delta_	= -1;
		}
	...
	void stop()
	{
		...
		//background_		= make_safe<frame_producer>(background_);
		frame_number_		= 0;
		auto_play_delta_	= -1;
	...
	void set_foreground(safe_ptr<core::frame_producer> producer)
	{
		...
		foreground_	= std::move(producer);	//done to kill g++ warnings
...
void layer::load(const safe_ptr<frame_producer>& frame_producer, bool preview, int auto_play_delta)
{
...
boost::unique_future<std::wstring> layer::call(bool foreground, const std::wstring& param)
{
	return impl_->call(foreground, param);
}
