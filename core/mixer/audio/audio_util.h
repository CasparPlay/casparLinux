...
public:
	...
	boost::iterator_range<iter_t> channel(const std::wstring& channel_name) const
	{
		auto it = std::find(channel_layout_.channel_names.begin(), channel_layout_.channel_names.end(),
										channel_name);
...
template<typename SampleT, typename Iter>
multichannel_view<SampleT, Iter> make_multichannel_view(const Iter& begin, const Iter& end, 
								const channel_layout& channel_layout)
{
