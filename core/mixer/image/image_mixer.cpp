...
private:
	...
	void draw(std::vector<layer>&&	layers, safe_ptr<device_buffer>& draw_buffer, const video_format_desc& format_desc)
	{
	...
	void draw_layer(layer&&	layer, safe_ptr<device_buffer>&	draw_buffer, 
			std::shared_ptr<device_buffer>& layer_key_buffer, const video_format_desc& format_desc)
	{				
	...
	void draw_mixer_buffer(safe_ptr<device_buffer>&	draw_buffer, std::shared_ptr<device_buffer>&& source_buffer, 
						   blend_mode   blend_mode = blend_mode::normal)
	{
		...
		draw_params.pix_desc.planes = list_of(pixel_format_desc::plane(source_buffer->width(), source_buffer->height(), 4)).to_container(draw_params.pix_desc.planes);	// to_container() is added to fix gcc compiler err
		draw_params.textures		= list_of(source_buffer).to_container(draw_params.textures);
...
public:
	...
	void visit(write_frame& frame)
	{
		...
		std::vector<safe_ptr<device_buffer>> tmp = frame.get_textures();
		std::vector<safe_ptr<device_buffer>>::iterator it2 = tmp.begin();
		// 
		// item.textures	= frame.get_textures();
		// the following for loop is introduced to avoid above assignment which gives compilation error.
		// since item.textures is basically a vector of safe_ptr<device_buffer> so we iterate over them
		// and manually assign all the safe_ptrs
		//
		for(std::vector<safe_ptr<device_buffer>>::iterator it = item.textures.begin();it!=item.textures.end();++it) {
			it = it2; ++it2;
		}
