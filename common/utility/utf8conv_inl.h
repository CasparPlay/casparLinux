...
inline std::wstring UTF16FromUTF8(const std::string & utf8)
{
   ...
   /* if (utf8.empty())
	...
	const DWORD conversionFlags = -1; // MB_ERR_INVALID_CHARS;
	
    //
    // Get length (in wchar_t's) of resulting UTF-16 string
    //
	const int utf16Length = utf8.length();
	...
	if (ret == -1) {
        ...
        throw utf8_conversion_error(
            (error == E2BIG) ? 
			"There's not sufficient room at outbuf" : (error == EILSEQ) ? 
				"An Invalid multibyte sequence has been encountered at input.":
                "An incomplete multibyte sequence has been encountered at input.", 
...
inline std::string UTF8FromUTF16(const std::wstring & utf16)
{
    ...
    //
	const int utf8Length = utf16.length();
		/*= ::WideCharToMultiByte(
