...
typedef struct tagTHREADNAME_INFO
{
	...
	int dwThreadID, dwFlags;
	//DWORD dwFlags; // reserved for future use, must be zero
} THREADNAME_INFO;

inline void SetThreadName(int dwThreadID,  const char * szThreadName)
{
	...
	/*__try
	{
		RaiseException( 0x406D1388, 0, sizeof(info)/sizeof(int), (int*)&info );
	}
...
void win32_exception::Handler(unsigned int errorCode, std::exception* pInfo) {
	/*switch(errorCode)
	{
...
/*win32_exception::win32_exception(const void& info) : message_("Win32 exception"), location_(info.ExceptionAddress), errorCode_(info.ExceptionCode)
{
