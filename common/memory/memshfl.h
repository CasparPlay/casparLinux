...
/* kill this code for now to avoid compilation err - write linux compatible code.
...
{   
	...
	{       
		memmove(reinterpret_cast<char*>(dest) + r.begin()*128, reinterpret_cast<const char*>(source) + r.begin()*128, r.size()*128);
