...
public:
	...
	void SetThreadPriority(int prio)
	{
		...
		if (prio == above_normal_priority_class) {
			pthread_setschedparam(pthread_self(), SCHED_RR, &param);
		}

		if (prio == normal_priority_class || prio == below_normal_priority_class) {
	...
	void set_priority_class(thread_priority p)
	{
		begin_invoke([=]
		{
			if (p == high_priority_class)
				...
				SetThreadPriority(high_priority_class);
			else if(p == above_normal_priority_class)
				SetThreadPriority(above_normal_priority_class);
			else if(p == normal_priority_class)
				SetThreadPriority(normal_priority_class);
			else if(p == below_normal_priority_class)
				SetThreadPriority(below_normal_priority_class);
		});
	}
