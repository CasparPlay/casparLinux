...
// Major changes in this file - frame_read_ has been disabled due to issue in compiling.
...
public:
	ogl_consumer(const configuration& config, const core::video_format_desc& format_desc, int channel_index) 
		...
		{			
			...
			return ffmpeg::filter(
				...
				sample_aspect_ratio, AV_PIX_FMT_BGRA, boost::assign::list_of(AV_PIX_FMT_BGRA),
	...
	{		
		...
		screen_x_		= 0;//devmode.dmPosition.x;
		screen_y_		= 0;//devmode.dmPosition.y;
...
//		screen_height_	= config_.windowed ? square_height_ : devmode.dmPelsHeight;
	...
	void run()
	{
		win32_exception::ensure_handler_installed_for_thread("ogl-consumer-thread");
		...
		try
		{
			...
			{			
				try
				{
					...
					{
						auto av_frame = safe_ptr<AVFrame>(av_frame_alloc(), 
		...
		if(ptr)
		{
			...
			else
			{
				tbb::parallel_for(0, av_frame->height, 1, [&](int y)
				{
					...
					memcpy(reinterpret_cast<char*>(ptr) + y * format_desc_.width * 4, av_frame->data[0] + y * av_frame->linesize[0], format_desc_.width * 4);
				});
			}
...
public:
	...
	virtual void initialize(const core::video_format_desc& format_desc, 
				const core::channel_layout& audio_channel_layout, int channel_index) override
	{
		consumer_.reset();
	...
	{
		if (consumer_)
			return consumer_->current_presentation_age_;
		else
			return 0;
...
safe_ptr<core::frame_consumer> create_consumer(const core::parameters& params)
{
	...
	config.screen_index = 0;//params.get(L"DEVICE", config.screen_index); changed to avoid err
