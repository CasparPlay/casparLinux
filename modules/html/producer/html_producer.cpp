...
// include is added to avoid Include dir issue.
	...
	namespace html {
		...
		private:
					...
					int height)
			{
				...
				// fast_memcpy(frame->image_data().begin(), buffer, width * height * 4); avoid compile err
				memcpy(frame->image_data().begin(), buffer, width * height * 4);
