...
{	
	...
	virtual safe_ptr<core::basic_frame> receive(int) override
	{
		...
		else
		{
			...
			//last_frame_ = field2; comment out for, to avoid compiler error
		...
		const core::parameters& params)
{
	...
	double speed = boost::lexical_cast<double>(params.get(L"SPEED", L"0.0"));
	double duration = boost::lexical_cast<double>(params.get(L"DURATION", L"0.0"));

	if(speed == 0 && duration == 0)
		return core::frame_producer::empty();

	int motion_blur_px = boost::lexical_cast<int>(params.get(L"BLUR", L"0"));
