...
public:
	...
	{		
		...
		last_frame_ = std::move(frame.first); //comment to avoid compilation fix
	...
	void try_decode_frame(int hints)
	{
		...
		if (video_decoder_->file_frame_number())
			file_frame_number = video_decoder_->file_frame_number();
...
safe_ptr<core::frame_producer> create_producer(const safe_ptr<core::frame_factory>& frame_factory, 
...
{		
	...
	} else
	{
		...
		// filename = env::media_folder() + L"\\" + tokens[1]; change to Linux path separator
...
//	auto start		= params.get(L"SEEK", std::uint32_t<std::uint32_t>(0)); changed to the following to avoid compilation error
	auto start		= 0;//boost::lexical_cast<uint32_t>(params.get(L"SEEK", L"0"));
	//auto length		= params.get(L"LENGTH", std::numeric_limits<uint32_t>::max());
	auto length		= 0;//boost::lexical_cast<uint32_t>(params.get(L"LENGTH", L"4294967295"));
...
safe_ptr<core::frame_producer> create_thumbnail_producer(const safe_ptr<core::frame_factory>& frame_factory,
		const core::parameters& params)
{		
