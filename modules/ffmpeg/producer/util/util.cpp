		...
		for(int n = 0; n < static_cast<int>(desc.planes.size()); ++n)
		{
			...
			if(decoded_linesize != static_cast<int>(plane.linesize))
			{
				...
				tbb::parallel_for<size_t>(0, desc.planes[n].height, [&](size_t y)
				{
					...
					//fast_memcpy(result + y*plane.linesize, decoded + y*decoded_linesize, plane.linesize); comment to avoid compiler err
				});
			}
			else
			{
				//fast_memcpy(result, decoded, plane.size); comment out to avoid compiler err
				memcpy(result, decoded, plane.size);
			}
...
bool is_valid_file(const std::wstring filename, const std::vector<std::wstring>& invalid_exts)
{
	...
	std::ifstream file(filename2);	// parameter
...
bool try_get_duration(const std::wstring filename, std::int64_t& duration, boost::rational<std::int64_t>& time_base)
{
	...
	std::shared_ptr<AVFormatContext> context(weak_context, av_close_input_file); //avoid compilation err
	//std::shared_ptr<AVFormatContext> context;//(&weak_context, avformat_close_input);
