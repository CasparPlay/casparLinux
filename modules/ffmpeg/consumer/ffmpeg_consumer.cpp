...
//		auto st = av_new_stream(oc_.get(), 0);	change it with avformat_new_stream to fix compile err
	...
	std::shared_ptr<AVStream> add_audio_stream(std::vector<option>& options)
	{
		...
		auto st = av_new_stream(oc_.get(), 1);	//change interface with avformat_new_stream()
		//auto st = avformat_new_stream(oc_.get(), NULL);
		if(!st)
	...
	byte_vector convert_audio(core::read_frame& frame, AVCodecContext* c)
	{
		if(!swr_) 		
			...
			c->sample_rate, format_desc_.audio_sample_rate, c->sample_fmt, AV_SAMPLE_FMT_S32));
...
public:
	...
	{
		if (consumer_)
			return consumer_->current_encoding_delay_;
		else
			return 0;
